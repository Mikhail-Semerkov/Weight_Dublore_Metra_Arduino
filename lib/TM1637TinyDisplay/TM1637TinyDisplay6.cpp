...
void TM1637TinyDisplay6::showString(const char s[], uint8_t length, uint8_t pos, uint8_t posdot)
{
  // digits[MAXDIGITS] output array to render
  memset(digits,0,sizeof(digits));
  ...
  if (strlen(s) <= MAXDIGITS) {
    ...
    if (strlen(posdot) >= 0 && strlen(posdot) != -1 && strlen(posdot) != false)   //&& strlen(posdot) != false
    {
        digits[posdot] += 0b10000000;
    }
